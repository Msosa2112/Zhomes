...
const CONTENT_MAP = {
    "Email Broker Admin": {
        ...
        subject: "Nueva solicitud de Realtor pendiente",
        html: `
    ...
    "Email Pending Realtor": {
        to: '["{{$json.body.record.agent_email}}"]',
        subject: "Nueva Solicitud de Sesión",
        html: `
    ...
    "Email Completed Cliente": {
        to: '["{{$json.body.record.client_email}}"]',
        subject: "¿Te encantó la propiedad?",
        html: `
    ...
    "Email Preapproved Cliente": {
        to: '["{{$json.body.record.email}}"]',
        subject: "Pre-aprobación completada",
        html: `
    ...
    "Email Offer Cliente": {
        to: '["{{$json.body.record.email}}"]',
        subject: "Oferta Formal Sometida",
        html: `
    ...
    "Email Closing Cliente": {
        to: '["{{$json.body.record.email}}"]',
        subject: "¡Felicidades, preparate para el cierre!",
        html: `
...
files.forEach(file => {
    ...
    data.nodes = data.nodes.map(node => {
        if (CONTENT_MAP[node.name]) {
            const map = CONTENT_MAP[node.name];
            const cleanHtml = ZHOMES_TEMPLATE(map.html).replace(/"/g, '\\"').replace(/\n/g, '');
            
            // Completely override jsonBody to be safe!
  ...
  "html": "${cleanHtml}"
            ...
            changed = true;
            console.log(`Rebuilt strictly: ${node.name}`);
        }
        return node;
    });
...
console.log(`Re-built ${changedFiles} files cleanly.`);
